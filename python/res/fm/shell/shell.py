...
class Shell(object):
    ...
    def __deleteFile(filename):
        ...
        if uid == os.getuid():
            ...
            sys.stderr.write("Sorry you are not owner of file:%s - not deleted\n" % filename)
        ...
        if uid == os.getuid():
            if os.path.islink( dirname ):
                ...
                try:
                    ...
                    if e.errno == 39:
                        sys.stderr.write("Failed to remove directory:%s - not empty\n" % dirname)
                    else:
                        raise
        else:
            sys.stderr.write("Sorry you are not owner of directory:%s - not deleted\n" % dirname)
