...
static bool enkf_config_node_has_container(const enkf_config_node_type * node , enkf_fs_type * fs , node_id_type node_id) {
  ...
  for (int inode=0; inode < vector_get_size( node->container_nodes ); inode++) {
    enkf_config_node_type * child_node = (enkf_config_node_type *)vector_iget( node->container_nodes , inode );
    bool has_child;
...
static enkf_config_node_type * enkf_config_node_alloc__(enkf_var_type  var_type, ert_impl_type  impl_type, const char * key, bool forward_init) {
    enkf_config_node_type * node = (enkf_config_node_type *)util_malloc( sizeof *node );
...
enkf_config_node_type * enkf_config_node_new_container( const char * key ) {
  enkf_config_node_type * config_node = enkf_config_node_alloc__( INVALID_VAR , CONTAINER , key , false);
...
const char * enkf_config_node_iget_container_key( const enkf_config_node_type * config_node , int index) {
  const enkf_config_node_type * child_node = (const enkf_config_node_type *)vector_iget_const( config_node->container_nodes , index );
...
enkf_config_node_type * enkf_config_node_alloc_field( const char * key , ecl_grid_type * ecl_grid, field_trans_table_type * trans_table, bool forward_init) {
  enkf_config_node_type * config_node = enkf_config_node_alloc__( INVALID_VAR , FIELD , key , forward_init);
...
void *  enkf_config_node_get_ref(const enkf_config_node_type * node) { // CXX_CAST_ERROR
  return node->data;
}
