...
custom_kw_config_set_type * custom_kw_config_set_alloc() {
    custom_kw_config_set_type * set = (custom_kw_config_set_type *)util_malloc(sizeof * set);
...
void custom_kw_config_set_add_config(custom_kw_config_set_type * set, const custom_kw_config_type * config) {
    pthread_rwlock_wrlock(& set->rw_lock);
    {
        ...
        stringlist_type * formatted_keys = (stringlist_type *)(stringlist_type *) hash_get(set->config_set, name);
    ...
    pthread_rwlock_rdlock(& set->rw_lock);
    {
        ...
        if (stream) {
            ...
            for (int i = 0; i < stringlist_get_size(keys); i++) {
                ...
                stringlist_type * formatted_keys = (stringlist_type *)(stringlist_type *) hash_get(set->config_set, key);
